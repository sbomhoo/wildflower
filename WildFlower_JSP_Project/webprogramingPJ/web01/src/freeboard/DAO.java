...
public class DAO {
	...
	public ArrayList<VO> getMemberList() {
		...
		try {
			sql = "SELECT NUM, id, TITLE, ndate, HIT, INDENT from freeboard order by ref desc, step asc";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
...
//	
	...
	public VO getView(int idx) {
		...
		try {
			sql = "SELECT id,TITLE, MEMO, ndate, HIT, REF, INDENT, STEP FROM freeboard WHERE NUM=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, idx);
			rs = pstmt.executeQuery();
	...
	public void modify(VO vo, int idx) {
		...
		try {
			sql = "UPDATE freeboard SET TITLE=?, MEMO=? where NUM=?";
			pstmt = con.prepareStatement(sql);
	...
	public void insertReply(VO vo, int ref, int indent, int step) {
		...
		try {
			sql = "INSERT INTO freeboard(id, TITLE, MEMO, REF, INDENT, STEP) "+
					"VALUES(?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
